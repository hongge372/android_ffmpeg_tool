...
//        av_log(NULL, AV_LOG_ERROR, "before set pts_time chg_rate:=%lf ", chg_rate );
        ...
        if(abort_flg == 0 &&
           ...
           pts_time < (start_time + duration)){
            av_log(NULL, AV_LOG_ERROR, "dump stu pts---after -> pkt.pts:=%lld\n", pkt.pts );
            pkt.dts = pkt.pts -= dec_pts;
            av_log(NULL, AV_LOG_ERROR, "dump stu pts---after dec_pts -> pkt.pts:=%lld pkt.dts:=%lld \n", pkt.pts , pkt.dts);
        

            av_log(NULL, AV_LOG_ERROR, " qc cut check, pts check success, input !  pts_time:= %lf start_time:=%ld start_time + duration:=%ld \n",  pts_time, start_time, start_time + duration);
